...
// Get the data
TFile f1("../Step3_MC/MC_D2PiMuMu12_MagDown_NTuples_fin.root","read"); 
//

void Fit_D2Pimumu_Mass( RooDataSet* Data, RooRealVar* D_MM, Double_t MassMin, Double_t MassMax, Double_t Fit_Results[11], Double_t Fit_Result_Errors[10], TString Param_Names[11],  TFile &fOutput, TString Title = "") {
  ...
  RooRealVar alpha("#alpha", "#alpha", 2.24735,.1,10.); 
  RooRealVar n("n", "n", 0.934334,0.,10.);
 ...
 // RooCBShape Ds_CB2("Ds_CB2", "Ds_CB2", *D_MM, mean_Ds, sigma2, alpha, n); 
  ...
  RooAddPdf D_Bgd_CB("D_Bgd_PDF", "D_Bgd_PDF", RooArgList(D_CB,CombBG_PDF ), RooArgList(frac1));
  //Ds, D+, BG
  // Yields
 ...
 RooAddPdf PDF("PDF", "PDF", RooArgList(D_Ds_PDF, CombBG_PDF), RooArgSet(D_Yield, CombBG_Yield ));
  ...
  // Perform the fit
  // PDF
  D_Bgd_CB.fitTo(*Data, Extended(1), RooFit::Save( true ), PrintEvalErrors(-1), Minos(kTRUE) );
  // Plot the results
  ...
  //PDF
  D_Bgd_CB.plotOn(pMass) ;
  RooHist* hMass_Pull = pMass->pullHist(); 
  Double_t Fit_chi2 = pMass->chiSquare(nFitParams);
  D_Bgd_CB.plotOn(pMass, Components(D_CB), LineColor(kRed));
  //D_CB.plotOn(pMass, Components(Ds_CB), LineColor(kOrange-3));
  //D_CB.plotOn(pMass, Components(CombBG_PDF), LineColor(kGray+2)) ;
  //D_CB.plotOn(pMass, Components(D_Ds_PDF), LineColor(kViolet)) ;
  pMass->Draw();
  //Label->Draw("SAME");
  cMass.Write();
...
void Main() 
{
  ...
  TFile fOutput("Results/MC_Output.root","recreate");   
  ...
  // Perform the fit
  Fit_D2Pimumu_Mass( All_Data, D_MM, MassMin, MassMax, All_Fit_Results, All_Fit_Result_Errors, All_Param_Names, fOutput, "All" );
 
  ofstream myfile;
  myfile.open("Results/MC_Output_D2PimumuOSTuple.txt",ios::out | ios::trunc);
  myfile << "Results" << endl;
  myfile << "_______" << endl;
